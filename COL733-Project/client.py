...
class Client:
    def __init__(self, name: str, host: str, port: int,message_handler=None, username: str = "Testing", password: str="password") -> None:
        self.name: str = name
        self.server=None
        ...
        self.incoming_key=self.security.public_key
        
        self.message_handler=message_handler
        
        self.lock=threading.Lock()
        
        # threading.Thread(target=self.watch_for_messages, daemon=True).start()
    ...
    def watch_for_messages(self) -> None:
        """Watches for incoming messages by sending get messages for incoming key."""
        while True:
            try:
                message ={
                    'type': "get",
                    'key': self.security.serialize_key(self.incoming_key)
                }
                reply=self.send_prompt(message)
                message={
                    'type': "put",
                    'key': self.security.serialize_key(self.incoming_key),
                    'data': ""
                ...
                data=reply["data"]
                threading.Thread(target=self.handle_messages,args=(data,)).start()
                time.sleep(1)
            except Exception as e:
                self.logger.error(f"Failed to watch for messages: {e}")    
